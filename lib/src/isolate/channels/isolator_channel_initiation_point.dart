...
class IsolatorChannelInitiationPoint with DisposableMixin implements IsolatorChannel {
  ...
  Result<void> send(dynamic item) {
    ...
    try {
      ...
      return ExceptionResult(exception: ex, stackTrace: st, message: const FixedOration(message: 'An error occurred while trying to send an object from one thread to another, verify that it does not have native attributes'));
    }
  }
}
